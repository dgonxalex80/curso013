...
if (file.exists("img/recurso3.png")) knitr::include_graphics("img/recurso3.png")
```
...
S=\left\{
\begin{aligned}
&(s,s,s),(s,s,c),(s,c,s),(s,c,c),\\
&(c,s,s),(c,s,c),(c,c,s),(c,c,c)
\end{aligned}
\right\}
$$
...
knitr::include_graphics("img/dosmonedas.png")
```
...
knitr::include_graphics("img/undado.png")
```
...
x=1:6
fx=rep(1,6)/6
plot(x,fx,pch=19, ylim=c(0,0.18), las=1, xlab="puntos", ylab="f(x)", col=c7)
...
fx=rep(1,6)/6
Fx=cumsum(fx)
plot(x,Fx, las=1, ylab="F(x)", xlab="puntos",pch=19, ylim=c(0,1), col=c7)
...
El restaurante “Asados y algo más” solo presta servicio mediante reservas. De acuerdo con los registros diarios de los últimos diez años, se sabe que el 30 % de las personas que reservan no llegan al restaurante.

Para la franja de almuerzo, comprendida entre las 12:00 m. y las 2:00 p. m., el restaurante acepta 10 reservas. Suponga que la llegada de cada persona es independiente de las demás.

Sea \(X\) la variable aleatoria que representa el número de personas, entre las 10 que reservaron, que efectivamente llegan al restaurante durante esta franja horaria.
...
knitr::include_graphics("img/restaurante.png")
```
</div>
</div>
...
f_X(x)=
\begin{cases}
\displaystyle {10\choose x}(0.7)^x(0.3)^{10-x},
...
```{r}
x=0:10
fx=choose(10,x)*0.70^x*0.30^(10-x)
Fx=cumsum(fx)

tabla=data.frame(x, fx, Fx)
knitr::kable(tabla)

#DT::datatable(head(tabla, 11),fillContainer = FALSE, options = list(pageLength = 12))
```
...
x=0:10
fx=choose(10,x)*0.70^x*0.30^(10-x)
plot(x,fx, las=1, ylab="f(x)", xlab="número personas que asisten", pch=19, col=c7)
...
fx=choose(10,x)*0.70^x*0.30^(10-x)
Fx=cumsum(fx)
plot(x,Fx, las=1, ylab="F(x)", xlab="número personas que asisten",pch=19, ylim=c(0,1), col=c7)
lines(x, Fx, type="s", col="#F7D8D8")
grid()
...
```{r}
x=0:10
fx=round(choose(10,x)*0.70^x*0.30^(10-x),6)
Fx=round(cumsum(fx),4)

cat("P(X=0)  = ",fx[1], "\n")
cat("P(X=7) = ", fx[8], "\n")
cat("P(X<7) = P(X<=6) = ", Fx[7], "\n")
cat("P(X>7) = 1 - P(X<=7) =", 1-Fx[8])
```
...
<div style="width: 30%; text-align: center;">
```{r, echo=FALSE, out.width="100%", }
knitr::include_graphics("img/lavadoras.png")
```
...
x=0:20
plot(x,fx(x), type="l", col=c41, lwd=5, las=1)

# área de integración
x1=seq(0,20,0.01)
f1=fx(x1)
polygon(c(0,x1,20),c(0,f1,0),col=c42)
```
